...
<div @class([
    ...
    @if ($activity)
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between border-b-[3px] border-black bg-white p-3 flex-shrink-0" @if ($isPollingActive) wire:poll.1000ms @endif>
            <div class="flex items-center gap-3">
                <div class="flex items-center justify-center w-8 h-8 bg-neutral-900 border-2 border-black shadow-[2px_2px_0_#000000] rounded">
                    ...
                    @else
                        <svg class="w-4 h-4 text-[#22d3ee]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="3" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M4.5 12.75l6 6 9-13.5" /></svg>
                    @endif
                </div>
                <div>
            ...
            <div class="mt-2 md:mt-0 flex items-center gap-2">
                ...
                @else
                    <div class="flex items-center gap-2 px-2 py-1 bg-[#22d3ee]/10 border-2 border-[#22d3ee] text-[#0891b2] dark:text-[#67e8f9] text-[10px] font-black uppercase tracking-widest rounded-sm">
        ...
        observer.observe($el, {
            ...
            @class([
                'flex flex-col w-full px-4 py-3 overflow-y-auto bg-[#05020a] text-[#22d3ee] scrollbar',
                'flex-1 min-h-[300px]' => $fullHeight,
                'max-h-[500px] min-h-[200px]' => !$fullHeight,
            ])>
            <pre class="font-mono text-[11px] leading-relaxed whitespace-pre-wrap selection:bg-[#22d3ee] selection:text-black" style="text-shadow: 0 0 3px rgba(34, 211, 238, 0.45);" @if ($isPollingActive) wire:poll.1000ms="polling" @endif>{{ RunRemoteProcess::decodeOutput($activity) }}</pre>
        </div>
    @else
        @if ($showWaiting)
