        ...
        @else
            <div class="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-4">
                <div class="rounded border-2 border-black bg-neutral-50 dark:bg-neutral-900/60 p-3 shadow-[2px_2px_0px_#000000]">
                    ...
                    <div class="mt-1 text-2xl leading-none font-black text-black dark:text-white">{{ data_get($sl1Network, 'summary.peers', 0) }}</div>
                </div>
                <div class="rounded border-2 border-black bg-[#22d3ee]/10 p-3 shadow-[2px_2px_0px_#000000]">
                    ...
                    <div class="mt-1 text-2xl leading-none font-black text-black dark:text-white">{{ data_get($sl1Network, 'summary.verified', 0) }}</div>
                </div>
                <div class="rounded border-2 border-black bg-[#a855f7]/10 p-3 shadow-[2px_2px_0px_#000000]">
                    ...
                    <div class="mt-1 text-2xl leading-none font-black text-black dark:text-white">{{ data_get($sl1Network, 'summary.observed_events', 0) }}</div>
                </div>
                <div class="rounded border-2 border-black bg-[#f59e0b]/10 p-3 shadow-[2px_2px_0px_#000000]">
                    ...
                    <div class="mt-1 text-2xl leading-none font-black text-black dark:text-white">{{ data_get($sl1Network, 'summary.discovery_candidates', 0) }}</div>
                </div>
            </div>
            ...
            <div class="mt-4 grid gap-4 xl:grid-cols-3">
                <div class="rounded border-2 border-black bg-neutral-50 dark:bg-neutral-900/50 p-4 shadow-[2px_2px_0px_#000000]">
                    <div class="flex items-start justify-between gap-3">
                        <div class="min-w-0">
                            ...
                            <div class="mt-1 text-sm font-black text-black dark:text-white break-all">
                                {{ data_get($sl1Network, 'local.issuer') }}
                            </div>
                        </div>
                        <span class="shrink-0 px-2 py-0.5 text-[9px] font-black uppercase tracking-wider rounded border border-black text-[#22d3ee] bg-[#22d3ee]/10">
                            {{ strtoupper(data_get($sl1Network, 'local.status', 'unknown')) }}
                        </span>
                    </div>
                    <div class="mt-4 grid gap-2 text-[10px] font-bold uppercase tracking-widest text-neutral-500 dark:text-neutral-400">
                        <div class="grid grid-cols-[90px_minmax(0,1fr)] gap-3">
                            <span>Node ID</span>
                            <span class="text-black dark:text-white normal-case tracking-normal break-all">{{ data_get($sl1Network, 'local.node_id') ?: 'not published yet' }}</span>
                        </div>
                        <div class="grid grid-cols-[90px_minmax(0,1fr)] gap-3">
                            <span>Algorithm</span>
                ...
                <div class="rounded border-2 border-black bg-neutral-50 dark:bg-neutral-900/50 p-4 shadow-[2px_2px_0px_#000000] xl:col-span-2">
                    <div class="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between sm:gap-3 mb-3">
                        <div>
                            ...
                            <div class="rounded border border-black bg-white dark:bg-black/30 p-3">
                                <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                                    <div class="min-w-0">
                                        ...
                                        <div class="text-[10px] font-bold text-neutral-500 uppercase tracking-widest break-all">{{ data_get($peer, 'issuer') }}</div>
                                    </div>
                                ...
                                <div class="mt-2 text-[10px] font-bold text-neutral-500 break-all">
                        ...
                        <div class="rounded border border-dashed border-black bg-white dark:bg-black/30 p-3">
                            <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                                <div class="min-w-0">
                                    ...
                                    <div class="text-[10px] font-bold text-neutral-500 uppercase tracking-widest break-all">{{ data_get($candidate, 'issuer') ?: 'issuer pending' }}</div>
                                </div>
                                <div class="flex flex-wrap gap-1.5 shrink-0">
                            ...
                            <div class="mt-2 grid grid-cols-2 lg:grid-cols-4 gap-2 text-[10px] font-bold uppercase tracking-widest text-neutral-500 dark:text-neutral-400">
                                <div>
                                    <div>Request</div>
                                    <div class="text-black dark:text-white normal-case tracking-normal break-all">{{ data_get($candidate, 'request_id') }}</div>
                                </div>
                                <div>
                                    <div>Status</div>
            ...
            <div class="mt-4 rounded border-2 border-black bg-neutral-50 dark:bg-neutral-900/50 p-4 shadow-[2px_2px_0px_#000000]">
                <div class="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between sm:gap-3 mb-3">
                    <div>
                        <div class="text-[9px] font-black uppercase tracking-widest text-neutral-500">Election Timeline</div>
                        <div class="text-sm font-black text-black dark:text-white">Simple L1 Failover</div>
                    </div>
                    <span class="text-[9px] font-black uppercase tracking-widest text-neutral-500">Observation -> Decision -> Control</span>
                </div>

                @if (! data_get($sl1Network, 'failover.available'))
                    <div class="rounded border-2 border-dashed border-black bg-white dark:bg-black/20 p-4 text-xs font-bold text-neutral-500 dark:text-neutral-400">
                        {{ data_get($sl1Network, 'failover.reason', 'Simple L1 failover timeline is not available yet.') }}
                    </div>
                @else
                    <div class="grid gap-3 lg:grid-cols-3">
                        <div class="rounded border border-black bg-white dark:bg-black/30 p-3">
                            <div class="text-[9px] font-black uppercase tracking-widest text-neutral-500">Current Target</div>
                            <div class="mt-1 text-base font-black text-black dark:text-white break-all">
                                {{ data_get($sl1Network, 'failover.current_target') ?: 'not recorded' }}
                            </div>
                            <div class="mt-1 text-[10px] font-bold uppercase tracking-widest text-neutral-500 break-all">
                                {{ data_get($sl1Network, 'failover.domain') }}
                            </div>
                        </div>

                        <div class="rounded border border-black bg-white dark:bg-black/30 p-3">
                            <div class="text-[9px] font-black uppercase tracking-widest text-neutral-500">Latest Recommendation</div>
                            <div class="mt-1 text-base font-black text-black dark:text-white">
                                {{ strtoupper(data_get($sl1Network, 'failover.latest_decision.recommendation', 'no data')) }}
                            </div>
                            <div class="mt-1 text-[10px] font-bold uppercase tracking-widest text-neutral-500">
                                {{ data_get($sl1Network, 'failover.latest_decision.reason', 'waiting for first decision') }}
                            </div>
                        </div>

                        <div class="rounded border border-black bg-white dark:bg-black/30 p-3">
                            <div class="text-[9px] font-black uppercase tracking-widest text-neutral-500">Evidence Hash</div>
                            <div class="mt-1 font-mono text-sm font-black text-black dark:text-white">
                                {{ data_get($sl1Network, 'failover.latest_decision.evidence_short', 'none') }}
                            </div>
                            <div class="mt-1 text-[10px] font-bold uppercase tracking-widest text-neutral-500">
                                {{ data_get($sl1Network, 'failover.latest_decision.decided_at', 'no decision yet') }}
                            </div>
                            <div class="mt-1 text-[10px] font-bold text-neutral-500 break-all">
                                package={{ data_get($sl1Network, 'failover.latest_decision.evidence_package_uuid', 'pending') }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-3 grid gap-3 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
                        <div class="rounded border border-black bg-white dark:bg-black/30 p-3">
                            <div class="mb-2 text-[9px] font-black uppercase tracking-widest text-neutral-500">Observed Node Status</div>
                            <div class="grid gap-2">
                                @forelse (data_get($sl1Network, 'failover.node_statuses', []) as $node)
                                    <div class="flex items-center justify-between gap-3 rounded border border-black/20 bg-neutral-50 dark:bg-neutral-950/60 p-2">
                                        <div class="min-w-0">
                                            <div class="text-xs font-black text-black dark:text-white truncate">{{ data_get($node, 'target_node') ?: data_get($node, 'target_ip') }}</div>
                                            <div class="text-[10px] font-bold text-neutral-500 break-all">{{ data_get($node, 'target_ip') }} · {{ data_get($node, 'health_source') }}</div>
                                        </div>
                                        <span @class([
                                            'shrink-0 px-2 py-0.5 text-[9px] font-black uppercase tracking-wider rounded border border-black',
                                            'text-[#22d3ee] bg-[#22d3ee]/10' => data_get($node, 'status') === 'healthy',
                                            'text-[#f53003] bg-[#f53003]/10' => data_get($node, 'status') !== 'healthy',
                                        ])>
                                            {{ strtoupper(data_get($node, 'status', 'unknown')) }}
                                        </span>
                                    </div>
                                @empty
                                    <div class="rounded border-2 border-dashed border-black bg-neutral-50 dark:bg-neutral-950/60 p-3 text-xs font-bold text-neutral-500">
                                        No node observations recorded yet.
                                    </div>
                                @endforelse
                            </div>
                        </div>
                        ...
                        <div class="rounded border border-black bg-white dark:bg-black/30 p-3">
                            <div class="mb-2 text-[9px] font-black uppercase tracking-widest text-neutral-500">Recent Evidence Timeline</div>
                            <div class="grid gap-2">
                                @forelse (data_get($sl1Network, 'failover.timeline', []) as $event)
                                    <div class="rounded border border-black/20 bg-neutral-50 dark:bg-neutral-950/60 p-2">
                                        <div class="flex flex-col gap-1 sm:flex-row sm:items-start sm:justify-between">
                                            <div class="min-w-0">
                                                <div class="text-xs font-black text-black dark:text-white">{{ data_get($event, 'title') }}</div>
                                                <div class="text-[10px] font-bold text-neutral-500 break-all">{{ data_get($event, 'detail') }}</div>
                                            </div>
                                            <div class="shrink-0 text-[10px] font-bold uppercase tracking-widest text-neutral-500">
                                                {{ data_get($event, 'at') ?: 'just now' }}
                                            </div>
                                        </div>
                                        @if (data_get($event, 'evidence_short'))
                                            <div class="mt-1 font-mono text-[10px] font-bold text-neutral-500">evidence={{ data_get($event, 'evidence_short') }}</div>
                                        @endif
                                        @if (data_get($event, 'evidence_package_uuid'))
                                            <div class="mt-1 text-[10px] font-bold text-neutral-500 break-all">package={{ data_get($event, 'evidence_package_uuid') }}</div>
                                        @endif
                                    </div>
                                @empty
                                    <div class="rounded border-2 border-dashed border-black bg-neutral-50 dark:bg-neutral-950/60 p-3 text-xs font-bold text-neutral-500">
                                        No failover decisions or observations recorded yet.
                                    </div>
                                @endforelse
                            </div>
                        </div>
