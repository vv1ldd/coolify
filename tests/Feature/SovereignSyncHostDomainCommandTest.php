...
test('host domain sync skips localhost ssh proxy sync for local server', function () {
    ...
    $this->artisan('sovereign:sync-host-domain', [
        ...
        ->expectsOutput('Local server #0 is configured for localhost SSH, but SSH is not required for Sovereign host-domain sync. Panel URL was updated; proxy sync was skipped.')
        ->assertExitCode(0);
